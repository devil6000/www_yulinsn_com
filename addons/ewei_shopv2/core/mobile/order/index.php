...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		if($show_status == 7){
		    //收礼订单
            $condition = " and gift_openid=:openid and ismr=0 and deleted=0 and uniacid=:uniacid and istrade=0";
        }else{
		...
		if( $show_status != "" ) 
		{
			...
			switch( $show_status ) 
			{
				...
				break;
                case 6: $condition .= " and order_type = 1";
                break;
                case 7: $condition .= " and order_type = 1";
                break;
			...
			if( !in_array($show_status, array(6,7))){
                $condition .= " and order_type = 0";
            }
		}
		else 
		{
			$condition .= " and userdeleted=0 and userdeleted = 0";
		}
	...
	public function detail() 
	{
		...
		if( $order['order_type'] == 1){
		    header("location:" . mobileUrl("order/gift/detail", array("id" => $order['id'])));
