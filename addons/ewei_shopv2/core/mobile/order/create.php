...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function submit() 
	{
		...
		if(!empty($_GPC['gift_mobile'])){
		    //收礼人手机号码
            $giftMember = pdo_fetch('select openid from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and uniacid=:uniacid', array(':mobile' => $_GPC['gift_mobile'], ':uniacid' => $_W['uniacid']));
            if(empty($giftMember)){
                show_json(0,'该会员不存在');
            }
            if($openid == $giftMember['openid']){
                show_json(0,'购买人不能为收礼人');
            }

            $order['order_type'] = 1;
            $order['gift_mobile'] = $_GPC['gift_mobile'];
            $order['gift_openid'] = $giftMember['openid'];
