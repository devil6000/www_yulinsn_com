...
class Op_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function finish() 
	{
        ...
        $order = pdo_fetch("select id,status,openid,couponid,price,refundstate,refundid,ordersn,price from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"]));
		if( empty($order) ) 
		{
			show_json(0, "订单未找到");
		}
